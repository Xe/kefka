...
func TestCut(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "char dedups repeated position selectors",
			args:       []string{"-c", "1,1,2"},
			stdin:      "abc\n",
			wantStdout: "ab\n",
		},
		...
		{
			name:       "duplicate field values at distinct positions are not deduped",
			args:       []string{"-d", ",", "-f", "1-3"},
			stdin:      "a,a,b\n",
			wantStdout: "a,a,b\n",
		},
		{
			name:       "repeated position selector emits field once",
			args:       []string{"-d", ",", "-f", "1,1"},
			stdin:      "a,b,c\n",
			wantStdout: "a\n",
		},
		{
			name:       "repeated position with same value at distinct positions",
			args:       []string{"-d", "X", "-f", "1,1"},
			stdin:      "aXa\n",
			wantStdout: "a\n",
		},
		{
			name:       "byte select single",
			args:       []string{"-b", "1"},
			stdin:      "a\tb\tc\n",
			wantStdout: "a\n",
		},
		{
			name:       "byte select range",
			args:       []string{"-b", "1-3"},
			stdin:      "a\tb\tc\n",
			wantStdout: "a\tb\n",
		},
		{
			name:       "char is rune-indexed for multibyte",
			args:       []string{"-c", "1"},
			stdin:      "λa\n",
			wantStdout: "λ\n",
		},
		{
			name:       "byte is byte-indexed for multibyte",
			args:       []string{"-b", "1"},
			stdin:      "λa\n",
			wantStdout: "\xce\n",
		},
		{
			name:       "byte with -n trims when range straddles char",
			args:       []string{"-b", "1", "-n"},
			stdin:      "λa\n",
			wantStdout: "\n",
		},
		{
			name:       "byte with -n keeps fully covered char",
			args:       []string{"-b", "1-2", "-n"},
			stdin:      "λa\n",
			wantStdout: "λ\n",
		},
		{
			name:       "byte with -n extends low to char start when high is char end",
			args:       []string{"-b", "2", "-n"},
			stdin:      "λa\n",
			wantStdout: "λ\n",
		},
		{
			name:       "byte with -n drops range entirely inside one char",
			args:       []string{"-b", "2", "-n"},
			stdin:      "λλ\n",
			wantStdout: "λ\n",
		},
		{
			name:       "byte with -n drops mid-char single byte in 3-byte char",
			args:       []string{"-b", "2", "-n"},
			stdin:      "你好\n",
			wantStdout: "\n",
		},
		{
			name:       "both -c and -f rejected",
			args:       []string{"-c", "1", "-f", "1"},
			wantErrSub: "only one type of list",
			wantErr:    true,
		},
		{
			name:       "both -b and -c rejected",
			args:       []string{"-b", "1", "-c", "1"},
			wantErrSub: "only one type of list",
			wantErr:    true,
		},
		{
			name:       "both -b and -f rejected",
			args:       []string{"-b", "1", "-f", "1"},
			wantErrSub: "only one type of list",
			wantErr:    true,
		},
		{
			name:       "no selector errors",
			args:       []string{},
			stdin:      "a\tb\n",
